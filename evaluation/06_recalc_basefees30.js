...
var csv = fs.readFileSync("recalc30.csv", "utf8");
var lines = csv.split("\r\n");
...
blocks[min].base_fee30_per_gas = parseInt(blocks[min].base_fee_per_gas);
...
for(var i = min + 1; i <= max; i++) {
	...
	var parent_base_fee_per_gas = parseInt(parent.base_fee30_per_gas);
	var parent_gas_used = parseInt(parent.gas_used30);
	...
	blocks[i].base_fee30_per_gas = expected_base_fee_per_gas;
	
	sql += "UPDATE blocks SET base_fee30_per_gas = " + expected_base_fee_per_gas + " WHERE number = " + i + ";\r\n";
}

fs.writeFileSync("recalc30.sql", sql, "utf8");
